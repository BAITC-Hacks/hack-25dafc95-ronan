...
Кнопка «Подтвердить добавление» — основной путь. Для текста можно поддержать только однозначные полные фразы, например «да, добавь» или «подтверждаю добавление», при наличии ровно одного актуального предложения, уже показанного этой сессии. Эта ветка выполняется сервером без решения LLM.
